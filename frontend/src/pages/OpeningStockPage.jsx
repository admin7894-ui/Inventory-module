...
const COLUMNS = [
  { key: 'opening_stock_id', label: 'Opening Stock Id' },
...
export default function OpeningStockPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['opening_stock_id'], formData)
      } else {
  ...
  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Opening Stock`:view==='edit'?`Edit Opening Stock`:`New Opening Stock`}
      ...
      <Field label="Opening Qty"><Input type="number" step="any"  value={formData.opening_qty} onChange={e => setField('opening_qty',e.target.value)} /></Field>
      ...
      <Field label="Total Value (Auto-calc)"><Input value={formData.total_value} readOnly /></Field>
      <Field label="Opening Date"><DateInput value={formData.opening_date} onChange={v => setField('opening_date',v)} /></Field>
  ...
  return (
    <>
      <DataTable
        title="Opening Stock"
        subtitle="Manage Opening Stock records"
        columns={COLUMNS}
        data={table.rows}
        total={table.total}
        page={table.page}
