...
export default function DesignationPage() {
  ...
  const table = useTableData(designationApi, 'designation')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['designation_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['designation_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Designation`:view==='edit'?`Edit Designation`:`New Designation`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Designation Id (Auto-gen)"><Input value={formData.designation_id} readOnly /></Field>
