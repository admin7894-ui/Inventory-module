...
export default function RolesPage() {
  ...
  const table = useTableData(rolesApi, 'roles')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['role_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['role_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Roles`:view==='edit'?`Edit Roles`:`New Roles`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Role Id (Auto-gen)"><Input value={formData.role_id} readOnly /></Field>
