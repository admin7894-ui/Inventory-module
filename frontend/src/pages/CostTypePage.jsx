...
const COLUMNS = [
  ...
  { key: 'cost_type_code', label: 'Cost Type Code' },
  { key: 'cost_type_name', label: 'Cost Type Name' },
...
export default function CostTypePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['cost_type_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['cost_type_id'])
  ...
  if (view !== 'list') {
    return (
      ...
      <Field label="Cost Type Code"><Input value={formData.cost_type_code} onChange={e => setField('cost_type_code',e.target.value)} /></Field>
      <Field label="Cost Type Name"><Input value={formData.cost_type_name} onChange={e => setField('cost_type_name',e.target.value)} /></Field>
  ...
  return (
    <>
      <DataTable
        title="Cost Type"
        subtitle="Manage Cost Type records"
        columns={COLUMNS}
        data={table.rows}
        total={table.total}
        page={table.page}
