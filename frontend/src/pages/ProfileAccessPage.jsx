...
export default function ProfileAccessPage() {
  ...
  const table = useTableData(profileAccessApi, 'profile_access')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['profile_access_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['profile_access_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Profile Access`:view==='edit'?`Edit Profile Access`:`New Profile Access`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Profile Access Id (Auto-gen)"><Input value={formData.profile_access_id} readOnly /></Field>
  ...
  return (
    <>
      <DataTable
        ...
        subtitle="Manage Profile Access records"
        columns={COLUMNS}
        data={table.rows}
        total={table.total}
        page={table.page}
