...
export default function LocationPage() {
  ...
  const table = useTableData(locationApi, 'location')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['location_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['location_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Location`:view==='edit'?`Edit Location`:`New Location`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Location Id (Auto-gen)"><Input value={formData.location_id} readOnly /></Field>
      <Field label="Location Name"><Input value={formData.location_name} onChange={e => setField('location_name',e.target.value)} /></Field>
      <Field label="Country"><Input value={formData.country} onChange={e => setField('country',e.target.value)} /></Field>
      <Field label="Currency"><Input value={formData.currency} onChange={e => setField('currency',e.target.value)} /></Field>
      <Field label="Timezone"><Input value={formData.timezone} onChange={e => setField('timezone',e.target.value)} /></Field>
      <Field label="Country Code"><Input value={formData.country_code} onChange={e => setField('country_code',e.target.value)} /></Field>
      <Field label="State"><Input value={formData.state} onChange={e => setField('state',e.target.value)} /></Field>
      <Field label="City"><Input value={formData.city} onChange={e => setField('city',e.target.value)} /></Field>
      <Field label="Postal Code"><Input value={formData.postal_code} onChange={e => setField('postal_code',e.target.value)} /></Field>
