...
const COLUMNS = [
  ...
  { key: 'Business Group Name', label: 'Business Group Name' },
...
export default function BusinessGroupPage() {
  ...
  const table = useTableData(businessGroupApi, 'business_group')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['bg_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['bg_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Business Group`:view==='edit'?`Edit Business Group`:`New Business Group`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Bg Id (Auto-gen)"><Input value={formData.bg_id} readOnly /></Field>
      <Field label="Business Group Name"><Input value={formData['Business Group Name'] ||''} onChange={e => setField('Business Group Name',e.target.value)} /></Field>
  ...
  return (
    <>
      <DataTable
        ...
        subtitle="Manage Business Group records"
        columns={COLUMNS}
        data={table.rows}
        total={table.total}
        page={table.page}
