...
export default function CompanyPage() {
  ...
  const table = useTableData(companyApi, 'company')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['company_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['company_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Company`:view==='edit'?`Edit Company`:`New Company`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Company Id (Auto-gen)"><Input value={formData.company_id} readOnly /></Field>
      ...
      <Field label="Company Name"><Input value={formData.company_name} onChange={e => setField('company_name',e.target.value)} /></Field>
      <Field label="Tan"><Input value={formData.TAN} onChange={e => setField('TAN',e.target.value)} /></Field>
      <Field label="Pan"><Input value={formData.PAN} onChange={e => setField('PAN',e.target.value)} /></Field>
      <Field label="Gst"><Input value={formData.GST} onChange={e => setField('GST',e.target.value)} /></Field>
      <Field label="Shop Act"><Input value={formData.shop_act} onChange={e => setField('shop_act',e.target.value)} /></Field>
      <Field label="Authorized Person"><Input value={formData.authorized_person} onChange={e => setField('authorized_person',e.target.value)} /></Field>
      <Field label="Incorporation Date"><DateInput value={formData.incorporation_date} onChange={v => setField('incorporation_date',v)} /></Field>
