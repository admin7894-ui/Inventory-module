...
export default function SecurityProfilePage() {
  ...
  const table = useTableData(securityProfileApi, 'security_profile')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['security_profile_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['security_profile_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Security Profile`:view==='edit'?`Edit Security Profile`:`New Security Profile`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Security Profile Id (Auto-gen)"><Input value={formData.security_profile_id} readOnly /></Field>
  ...
  return (
    <>
      <DataTable
        ...
        subtitle="Manage Security Profile records"
        columns={COLUMNS}
        data={table.rows}
        total={table.total}
        page={table.page}
