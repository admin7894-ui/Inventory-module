...
export default function SerialMasterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['serial_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['serial_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Serial Master`:view==='edit'?`Edit Serial Master`:`New Serial Master`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Serial Id (Auto-gen)"><Input value={formData.serial_id} readOnly /></Field>
  ...
  return (
    <>
      <DataTable
        title="Serial Master"
        subtitle="Manage Serial Master records"
        columns={COLUMNS}
        data={table.rows}
        total={table.total}
        page={table.page}
