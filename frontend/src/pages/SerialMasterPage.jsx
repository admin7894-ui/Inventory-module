  ...
  { key: 'serial_number',      label: 'Serial Number' },
  {
    ...
    render: v => (
      <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold uppercase ${
        v === 'AVAILABLE' ? 'bg-emerald-100 text-emerald-700' :
        v === 'ISSUED'    ? 'bg-rose-100 text-rose-700' :
        v === 'RESERVED'  ? 'bg-amber-100 text-amber-700' :
...
export default function SerialMasterPage() {
  const table = useTableData(serialMasterApi, 'serial_master')
  ...
  if (view === 'view' && selected) {
    return (
      <FormPage title="View Serial Master" onBack={handleBack} mode="view">
        <div className="card p-6 mb-4">
          <div className="mb-3 flex items-center gap-2 text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2">
            ...
            Serial records are system-generated. Use Opening Stock or Stock Adjustment to create serials.
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <Field label="Serial ID"><Input value={selected.serial_id} readOnly /></Field>
            ...
            <Field label="Status">
              <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${
                ...
                selected.status === 'ISSUED'    ? 'bg-rose-100 text-rose-700' :
  ...
  return (
    <DataTable
      title="Serial Master"
      subtitle="Read-only registry of all Serial Numbers — created via transactions only"
      columns={COLUMNS}
      data={table.rows}
      total={table.total}
      page={table.page}
