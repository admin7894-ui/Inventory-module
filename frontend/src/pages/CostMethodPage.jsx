...
const COLUMNS = [
  ...
  { key: 'cost_method_code', label: 'Cost Method Code' },
  { key: 'cost_method_name', label: 'Cost Method Name' },
...
export default function CostMethodPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['cost_method_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['cost_method_id'])
  ...
  if (view !== 'list') {
    return (
      ...
      <Field label="Cost Method Code"><Input value={formData.cost_method_code} onChange={e => setField('cost_method_code',e.target.value)} /></Field>
      <Field label="Cost Method Name"><Input value={formData.cost_method_name} onChange={e => setField('cost_method_name',e.target.value)} /></Field>
  ...
  return (
    <>
      <DataTable
        title="Cost Method"
        subtitle="Manage Cost Method records"
        columns={COLUMNS}
        data={table.rows}
        total={table.total}
        page={table.page}
