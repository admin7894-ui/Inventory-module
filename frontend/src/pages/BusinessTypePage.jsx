...
const COLUMNS = [
  ...
  { key: 'name', label: 'Name' },
  { key: 'departments_id', label: 'Departments Id' },
  { key: 'roles_id', label: 'Roles Id' },
...
export default function BusinessTypePage() {
  ...
  const table = useTableData(businessTypeApi, 'business_type')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['business_type_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['business_type_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Business Type`:view==='edit'?`Edit Business Type`:`New Business Type`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Business Type Id (Auto-gen)"><Input value={formData.business_type_id} readOnly /></Field>
      <Field label="Name"><Input value={formData.name} onChange={e => setField('name',e.target.value)} /></Field>
      <Field label="Department"><Select value={formData.departments_id} onChange={v => setField('departments_id',v)} options={dropdowns.departments?.map(r=>{return{value:r.dept_id,label:r.name||r.dept_id}})} /></Field>
      <Field label="Role"><Select value={formData.roles_id} onChange={v => setField('roles_id',v)} options={dropdowns.roles?.map(r=>{return{value:r.role_id,label:r.name||r.role_id}})} /></Field>
  ...
  return (
    <>
      <DataTable
        title="Business Type"
        subtitle="Manage Business Type records"
        columns={COLUMNS}
        data={table.rows}
        total={table.total}
        page={table.page}
