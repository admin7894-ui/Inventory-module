...
const COLUMNS = [
  ...
  { key: 'uom_type_code', label: 'Uom Type Code' },
  { key: 'uom_type_name', label: 'Uom Type Name' },
  { key: 'module_id', label: 'Module Id' }
]

export default function UomTypePage() {
  const navigate = useNavigate()
  const table = useTableData(uomTypeApi, 'uom_type')
  const v = useFormValidation('uom_type')
  ...
  const existingCodes = table.rows.map(r => r.uom_type_code)
  const setField = (k, val) => {
    setFormData(p => {
      const next = { ...p, [k]: val }
      if (k === 'uom_type_name') {
        next.uom_type_code = autoCode(val, 'UT_', existingCodes)
      }
      return next
    })
    v.clearError(k)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') await table.update(selected['uom_type_id'], formData)
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['uom_type_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View UOM Type`:view==='edit'?`Edit UOM Type`:`New UOM Type`}
        ...
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <Field label="Uom Type Id (Auto-gen)"><Input value={formData.uom_type_id} readOnly /></Field>
            <CompanyGroup formData={formData} setField={setField} errors={v.errors} handleBlur={v.handleBlur} />

            <Field label="Uom Type Name" required error={v.errors.uom_type_name}>
              <Input value={formData.uom_type_name} 
                onChange={e => setField('uom_type_name', e.target.value)}
                onBlur={() => v.handleBlur('uom_type_name', formData)}
                error={v.errors.uom_type_name} />
            </Field>

            <Field label="Uom Type Code" required error={v.errors.uom_type_code}>
              <Input value={formData.uom_type_code} 
                readOnly
                onBlur={() => v.handleBlur('uom_type_code', formData)}
                error={v.errors.uom_type_code}
  ...
  return (
    <>
      <DataTable
        title="UOM Type"
        subtitle="Manage UOM Type records"
        columns={COLUMNS}
        data={table.rows}
        total={table.total}
        page={table.page}
