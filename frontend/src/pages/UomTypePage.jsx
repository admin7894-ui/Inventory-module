...
export default function UomTypePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['uom_type_id'], formData)
      } else {
  ...
  if (view !== 'list') {
    return (
      ...
      <Field label="Uom Type Code"><Input value={formData.uom_type_code} onChange={e => setField('uom_type_code',e.target.value)} /></Field>
      <Field label="Uom Type Name"><Input value={formData.uom_type_name} onChange={e => setField('uom_type_name',e.target.value)} /></Field>
