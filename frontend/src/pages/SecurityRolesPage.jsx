...
const COLUMNS = [
  ...
  { key: 'profile_access_id', label: 'Profile Access Id' },
  { key: 'security_profile_id', label: 'Security Profile Id' },
...
export default function SecurityRolesPage() {
  ...
  const table = useTableData(securityRolesApi, 'security_roles')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['security_roles_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['security_roles_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Security Roles`:view==='edit'?`Edit Security Roles`:`New Security Roles`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Security Roles Id (Auto-gen)"><Input value={formData.security_roles_id} readOnly /></Field>
      <Field label="Profile Access"><Select value={formData.profile_access_id} onChange={v => setField('profile_access_id',v)} options={dropdowns.profileAccess?.map(r=>{return{value:r.profile_access_id,label:r.profile_access_id}})} /></Field>
      <Field label="Security Profile"><Select value={formData.security_profile_id} onChange={v => setField('security_profile_id',v)} options={dropdowns.securityProfile?.map(r=>{return{value:r.security_profile_id,label:r.profile_name||r.security_profile_id}})} /></Field>
      ...
      <Field label="Profile Code"><Input value={formData.profile_code} onChange={e => setField('profile_code',e.target.value)} /></Field>
      ...
      <Field label="Dept Id"><Input value={formData.dept_id} onChange={e => setField('dept_id',e.target.value)} /></Field>
      <Field label="Role Id"><Input value={formData.role_id} onChange={e => setField('role_id',e.target.value)} /></Field>
      <Field label="Designation"><Select value={formData.designation_id} onChange={v => setField('designation_id',v)} options={dropdowns.designation?.map(r=>{return{value:r.designation_id,label:r.name||r.designation_id}})} /></Field>
      <Field label="User Name"><Input value={formData.user_name} onChange={e => setField('user_name',e.target.value)} /></Field>
      <Field label="Password"><Input value={formData.password} onChange={e => setField('password',e.target.value)} /></Field>
  ...
  return (
    <>
      <DataTable
        ...
        subtitle="Manage Security Roles records"
        columns={COLUMNS}
        data={table.rows}
        total={table.total}
        page={table.page}
