...
export default function DepartmentsPage() {
  ...
  const table = useTableData(departmentsApi, 'departments')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['dept_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['dept_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Departments`:view==='edit'?`Edit Departments`:`New Departments`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Dept Id (Auto-gen)"><Input value={formData.dept_id} readOnly /></Field>
