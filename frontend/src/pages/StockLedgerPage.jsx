...
export default function StockLedgerPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['ledger_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['ledger_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Stock Ledger`:view==='edit'?`Edit Stock Ledger`:`New Stock Ledger`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Ledger Id (Auto-gen)"><Input value={formData.ledger_id} readOnly /></Field>
      ...
      <Field label="Transaction Id"><Input value={formData.transaction_id} onChange={e => setField('transaction_id',e.target.value)} /></Field>
      <Field label="Transaction Date"><DateInput value={formData.transaction_date} onChange={v => setField('transaction_date',v)} /></Field>
  ...
  return (
    <>
      <DataTable
        title="Stock Ledger"
        subtitle="Manage Stock Ledger records"
        columns={COLUMNS}
        data={table.rows}
        total={table.total}
        page={table.page}
