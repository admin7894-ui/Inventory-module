...
export default function LotMasterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'edit') {
        await table.update(selected['lot_id'], formData)
      } else {
  ...
  const handleDelete = async () => {
    await table.remove(confirmDelete['lot_id'])
    setConfirmDelete(null)
  }

  if (view !== 'list') {
    return (
      <FormPage title={view==='view'?`View Lot Master`:view==='edit'?`Edit Lot Master`:`New Lot Master`}
        onBack={handleBack} onSubmit={handleSubmit} loading={table.isCreating||table.isUpdating} mode={view}>
        <div className="card p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Field label="Lot Id (Auto-gen)"><Input value={formData.lot_id} readOnly /></Field>
      ...
      <Field label="Lot Number"><Input value={formData.lot_number} onChange={e => setField('lot_number',e.target.value)} /></Field>
  ...
  return (
    <>
      <DataTable
        title="Lot Master"
        subtitle="Manage Lot Master records"
        columns={COLUMNS}
        data={table.rows}
        total={table.total}
        page={table.page}
