...
const FormContext = createContext({ mode: 'edit', submitAttempted: false })
...
export function Select({ value, onChange, options = [], placeholder = '-- Select --', disabled, error, className, onBlur, displayValue }) {
  const { mode, submitAttempted } = useFormMode()
  const isView = mode === 'view'
  ...
  return (
    <select value={currentValue} onChange={e => onChange(e.target.value)} onBlur={onBlur} disabled={disabled}
      className={clsx(showError ? 'input-error' : 'input', 'cursor-pointer', className)}>
...
export function DateInput({ value, onChange, required, error, min, max, disabled }) {
  ...
  return (
    <input type="date" value={value || ''} onChange={e => onChange(e.target.value)}
      className={showError ? 'input-error' : 'input'} required={required} min={min} max={max} disabled={disabled} />
  )
}
...
export function Input({ value, onChange, type = 'text', placeholder, disabled, readOnly, error, className, ...props }) {
  ...
  return (
    <input
      ...
      readOnly={readOnly}
      className={clsx(showError ? 'input-error' : 'input', (readOnly || isView) && 'bg-gray-100 dark:bg-gray-800', shouldShowPlaceholder && 'text-gray-500 dark:text-gray-400', className)}
      {...props}
    />
  )
}
...
// ?????? Confirm Dialog ????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
export function ConfirmDialog({ open, title, message, onConfirm, onCancel, loading, confirmText = 'Delete' }) {
  if (!open) return null
  return (
...
export function Field({ label, required, error, children }) {
  const { submitAttempted } = useFormMode()
  const showError = !!error && submitAttempted
  return (
    <div>
...
export function FormPage({ title, subtitle, onBack, onSubmit, loading, children, mode = 'edit', disabled }) {
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit?.(e)
    setTimeout(focusFirstInvalid, 120)
    setTimeout(focusFirstInvalid, 420)
  }
  return (
    <FormContext.Provider value={{ mode, submitAttempted }}>
