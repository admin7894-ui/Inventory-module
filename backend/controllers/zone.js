...
exports.create = (req, res) => {
  try {
    ...
    // Uniqueness check for zone_code
    if ((db[TABLE]||[]).find(r => r.zone_code === body.zone_code))
      return res.status(400).json({ success:false, message:`Zone Code "${body.zone_code}" already exists` });
...
exports.update = (req, res) => {
  try {
    ...
    // Uniqueness check for zone_code
    if (req.body.zone_code && (db[TABLE]||[]).find(r => r.zone_code === req.body.zone_code && r[PK] !== req.params.id))
      return res.status(400).json({ success:false, message:`Zone Code "${req.body.zone_code}" already exists` });
