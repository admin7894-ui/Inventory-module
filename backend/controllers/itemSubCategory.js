...
const TABLE = 'item_sub_category';
const PK    = 'sub_category_id';
...
exports.create = (req, res) => {
  try {
    ...
    // Uniqueness check for sub_category_code
    if ((db[TABLE]||[]).find(r => r.sub_category_code === body.sub_category_code))
      return res.status(400).json({ success:false, message:`Sub Category Code "${body.sub_category_code}" already exists` });
...
exports.update = (req, res) => {
  try {
    ...
    // Uniqueness check for sub_category_code
    if (req.body.sub_category_code && (db[TABLE]||[]).find(r => r.sub_category_code === req.body.sub_category_code && r[PK] !== req.params.id))
      return res.status(400).json({ success:false, message:`Sub Category Code "${req.body.sub_category_code}" already exists` });
