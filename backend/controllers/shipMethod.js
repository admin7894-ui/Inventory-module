...
const TABLE = 'ship_method';
const PK    = 'ship_method_id';
...
exports.create = (req, res) => {
  try {
    ...
    // Uniqueness check for method_code
    if ((db[TABLE]||[]).find(r => r.method_code === body.method_code))
      return res.status(400).json({ success:false, message:`Method Code "${body.method_code}" already exists` });
...
exports.update = (req, res) => {
  try {
    ...
    // Uniqueness check for method_code
    if (req.body.method_code && (db[TABLE]||[]).find(r => r.method_code === req.body.method_code && r[PK] !== req.params.id))
      return res.status(400).json({ success:false, message:`Method Code "${req.body.method_code}" already exists` });
