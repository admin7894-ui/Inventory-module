...
exports.create = (req, res) => {
  try {
    ...
    // Uniqueness check for item_type_name
    if ((db[TABLE]||[]).find(r => r.item_type_name === body.item_type_name))
      return res.status(400).json({ success:false, message:`Item Type Name "${body.item_type_name}" already exists` });
...
exports.update = (req, res) => {
  try {
    ...
    // Uniqueness check for item_type_name
    if (req.body.item_type_name && (db[TABLE]||[]).find(r => r.item_type_name === req.body.item_type_name && r[PK] !== req.params.id))
      return res.status(400).json({ success:false, message:`Item Type Name "${req.body.item_type_name}" already exists` });
