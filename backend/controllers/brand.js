...
exports.create = (req, res) => {
  try {
    ...
    // Uniqueness check for brand_code
    if ((db[TABLE]||[]).find(r => r.brand_code === body.brand_code))
      return res.status(400).json({ success:false, message:`Brand Code "${body.brand_code}" already exists` });
...
exports.update = (req, res) => {
  try {
    ...
    // Uniqueness check for brand_code
    if (req.body.brand_code && (db[TABLE]||[]).find(r => r.brand_code === req.body.brand_code && r[PK] !== req.params.id))
      return res.status(400).json({ success:false, message:`Brand Code "${req.body.brand_code}" already exists` });
