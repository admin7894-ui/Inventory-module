...
exports.create = (req, res) => {
  try {
    ...
    if ((db[TABLE]||[]).find(r => r.category_code === body.category_code))
      return res.status(400).json({ success:false, message:`Category Code "${body.category_code}" already exists` });
...
exports.update = (req, res) => {
  try {
    ...
    // Uniqueness check for category_code
    if (req.body.category_code && (db[TABLE]||[]).find(r => r.category_code === req.body.category_code && r[PK] !== req.params.id))
      return res.status(400).json({ success:false, message:`Category Code "${req.body.category_code}" already exists` });
