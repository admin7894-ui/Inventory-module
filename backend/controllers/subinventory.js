...
exports.create = (req, res) => {
  try {
    ...
    if ((db[TABLE]||[]).find(r => r.subinventory_code === body.subinventory_code))
      return res.status(400).json({ success:false, message:`Subinventory Code "${body.subinventory_code}" already exists` });
...
exports.update = (req, res) => {
  try {
    ...
    if (req.body.subinventory_code && (db[TABLE]||[]).find(r => r.subinventory_code === req.body.subinventory_code && r[PK] !== req.params.id))
      return res.status(400).json({ success:false, message:`Subinventory Code "${req.body.subinventory_code}" already exists` });
